...
<head>
    ...
    <title>VENTAPLUS - Actualizar Categoría</title>
    ...
    <main class="w-full max-w-2xl px-4 sm:px-6 py-10 relative z-10">
        ...
        <div class="bg-zinc-950/45 border border-white/8 rounded-3xl p-6 sm:p-8 backdrop-blur-xl hover:border-white/10 transition-all duration-300 shadow-2xl">
            ...
            <form id="categoriaForm" action="CategoriaController" method="POST" class="space-y-5" onsubmit="return validarFormulario()">
                <!-- Hidden inputs (REQ-CAT-3.3: hidden id, POST to ?opcion=actualizar) -->
                <input type="hidden" name="opcion" value="actualizar">
                <input type="hidden" name="id" value="<c:out value='${categoria.id}'/>">

                <!-- Nombre (prefilled from ${categoria.nombre} — REQ-CAT-3.3) -->
                <div class="relative">
                    <div class="group relative flex items-center bg-white/[0.02] border border-white/8 rounded-2xl transition-all duration-300 hover:bg-white/[0.04] hover:border-emerald-500/35 focus-within:bg-zinc-950/70 focus-within:border-emerald-500 focus-within:ring-2 focus-within:ring-emerald-500/25">
                        ...
                        <input type="text" id="nombre" name="nombre" value="<c:out value='${categoria.nombre}'/>"
                ...
                <!-- Descripción (prefilled — REQ-CAT-3.3) -->
                <div class="relative">
                    <div class="group relative bg-white/[0.02] border border-white/8 rounded-2xl transition-all duration-300 hover:bg-white/[0.04] hover:border-emerald-500/35 focus-within:bg-zinc-950/70 focus-within:border-emerald-500 focus-within:ring-2 focus-within:ring-emerald-500/25">
                        ...
                        <textarea id="descripcion" name="descripcion" rows="4" maxlength="200"
                            class="peer w-full py-4 pl-12 pr-4 bg-transparent border-none outline-none text-sm text-zinc-100 placeholder-transparent resize-none"
                            placeholder="Descripción (opcional)"><c:out value='${categoria.descripcion}'/></textarea>
                ...
                <!-- Estado (REQ-CAT-3.3: A/I select toggle, prefilled from ${categoria.estado}) -->
                <div class="relative">
                    <div class="group relative flex items-center bg-white/[0.02] border border-white/8 rounded-2xl transition-all duration-300 hover:bg-white/[0.04] hover:border-emerald-500/35 focus-within:bg-zinc-950/70 focus-within:border-emerald-500 focus-within:ring-2 focus-within:ring-emerald-500/25">
                        <span class="absolute left-4 text-zinc-500 pointer-events-none transition-colors duration-300 group-focus-within:text-emerald-500">
                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        ...
                        <select id="estado" name="estado" class="w-full py-4 pl-12 pr-10 bg-transparent border-none outline-none text-sm text-zinc-100 appearance-none cursor-pointer focus:bg-zinc-950">
                            <option value="A" ${categoria.estado == 'A' ? 'selected' : ''} class="bg-zinc-950 text-zinc-100">Activo (A)</option>
                            <option value="I" ${categoria.estado == 'I' ? 'selected' : ''} class="bg-zinc-950 text-zinc-100">Inactivo (I)</option>
                        </select>
                        <span class="absolute right-4 text-zinc-500 pointer-events-none">
                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
